...
import { OotdCategory,  OotdTopBar, FilterContainer, OotdList, Container, TopDiv } from './ootd.styles'
...
const Ootd = () => {
    ...
    return (
        <TopDiv>
            <h1> OOTD </h1>
            <OotdTopBar><OotdCategory><h3>최신</h3><h3>인기</h3><h3>팔로잉</h3></OotdCategory> <FilterContainer><Filter/></FilterContainer> </OotdTopBar>
        
            <OotdList>
            {List.map((el, idx)=>{
                return <Container key={idx} onClick={goToCheckoutHandler}><img src={el} alt="" /></Container>
            })}
            </OotdList>
